...
const host = useFeed
  ? process.env.METRICS_SEARCH_HOST
  : process.env.SEARCH_HOST;
const apiKey = useFeed
  ? process.env.METRICS_SEARCH_API_KEY
  : process.env.SEARCH_API_KEY;

if (!host || !apiKey) {
  console.error(
    `Error: ${useFeed ? "METRICS_SEARCH_HOST/METRICS_SEARCH_API_KEY" : "SEARCH_HOST/SEARCH_API_KEY"} not configured`,
  );
  process.exit(1);
}
...
async function main() {
  console.error(
    `Using: ${useFeed ? "Feed/Metrics" : "Main"} Search (${host})\n`,
  );
    ...
    switch (command) {
      ...
      case "stats": {
        ...
        if (jsonOutput) {
          ...
          for (const [name, stats] of Object.entries(data.indexes)) {
            console.log(
              `  ${name}: ${stats.numberOfDocuments.toLocaleString()} docs, ${stats.isIndexing ? "INDEXING" : "ready"}`,
            );
          }
        }
        break;
      ...
      case "tasks": {
        ...
        if (jsonOutput) {
          ...
          console.log(
            `Total: ${data.total} | Showing: ${data.results.length}\n`,
          );
          for (const task of data.results) {
            ...
            console.log(
              `[${task.uid}] ${task.status.padEnd(10)} ${task.type.padEnd(20)} ${index.padEnd(25)} ${duration}`,
            );
            if (task.error) {
      ...
      case "index": {
        ...
        if (!subCommand) {
          ...
          if (jsonOutput) {
            ...
            console.log(
              `Documents: ${data.numberOfDocuments.toLocaleString()}`,
            );
            ...
            if (Object.keys(data.fieldDistribution).length > 15) {
              console.log(
                `  ... and ${Object.keys(data.fieldDistribution).length - 15} more fields`,
              );
            }
          }
          ...
          const data = await request(
            `/indexes/${commandArg}/settings/filterable-attributes`,
          );
          if (jsonOutput) {
            console.log(JSON.stringify(data, null, 2));
          } else {
          ...
          const data = await request(
            `/indexes/${commandArg}/settings/sortable-attributes`,
          );
          if (jsonOutput) {
            console.log(JSON.stringify(data, null, 2));
          } else {
          ...
          const data = await request(
            `/indexes/${commandArg}/settings/searchable-attributes`,
          );
          if (jsonOutput) {
            console.log(JSON.stringify(data, null, 2));
          } else {
          ...
          console.error(
            "Valid subcommands: settings, filterable, sortable, searchable",
          );
          process.exit(1);
        }
        break;
